...
public class WF {
    ...
    /**
     * This method implements the worst-fit memory allocation strategy.
     *
     * @param memoryBlock The available memory blocks.
     * @param processSize The size of the processes to be allocated.
     */
    public static void worstFit(int[] memoryBlock, int[] processSize) {
        ...
        for (int i = 0; i < processSize.length; i++) {
            ...
            for (int j = 0; j < memoryBlock.length; j++) {
                ...
                if (memoryBlock[j] >= processSize[i]) {
                    ...
                    // Calculate the difference between the memory block size and process size
                    int tempDif = memoryBlock[j] - processSize[i];

                    // Check if the current difference is larger than the previous difference
                    if (tempDif > dif) {
